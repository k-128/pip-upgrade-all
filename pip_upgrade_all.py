...
-uD, --upgrade-debug
    Upgrade all packages while printing pip output."""
    ...
    with Popen(cmd, stdout=SPROC_PIPE, universal_newlines=True) as p:
        for line in p.stdout:
            if ((line[:31] == "Requirement already up-to-date:"
                    or line[:30] == "Requirement already satisfied:")
                and i < len(PACKAGES)
            ):
                i += 1
...
def main():
    ...
    if len(sys.argv) == 1:
        print(f"Arg required, help: {HELP_CMD}")
        ...
        print(f"Invalid arg, help: {HELP_CMD}")
